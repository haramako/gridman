...
## 7. `db-server.ts` の PATCH は `deletedIds` を処理しない

`server/index.ts`（ファイルベース）と `server/db-server.ts`（SQLite）の PATCH エンドポイントには挙動差異がある。
ファイルサーバーは `{ rows, deletedIds }` の両方を処理するが、db サーバーは upsert のみで削除を実行しない。
db サーバー環境で行削除が機能しないバグが出た場合はここを疑う。

→ [[summaries/server]]

---

## 8. システム予約フィールドはユーザーカラムに使えない
