...
```ts
type FilterViewQuery = { type: 'filter'; from: string; filter?: FilterExpr; sort?: SortDef[]; columns?: string[] }
type UnionViewQuery  = { type: 'union'; sources: Array<{ from, columns?, filter? }> }
type LookupViewQuery = { type: 'lookup'; from: string; filter?: FilterExpr;
                         lookups: Array<{ column, from, as, fields }> }
type PageViewQuery   = { type: 'page'; from: string; filter?: FilterExpr; pageLayout?: string }

type ViewQuery = FilterViewQuery | UnionViewQuery | LookupViewQuery | PageViewQuery
```
...
行は単純な文字列キーの辞書。システム予約フィールド:
- `_id`: 行 ID（6 文字の乱数文字列）
- `_order`: ソート順（数値、挿入位置計算に使用）
- `_invalid`: バリデーション違反値の保存場所
- `_source`: Union ビューで元テーブルを識別
- `_sources`: Lookup ビューで複数元テーブルの ID を保持
