...
| `spec-feature-addition` | 2 | LIN-46, LIN-208 |
...
8. **`countermeasure: none` が14件に増加**: context-overload 系 lint 修正タスク（LIN-191, 193, 194, 195, 196, 197, 198）+ PRスキップ（LIN-208）。issue 記述ガイドラインとタイムアウト対策の整備が課題。
...
10. **ランタイム環境障害（LIN-206）**: `runtime_offline` と `queued_expired` の連続失敗後、ユーザーの再実行リクエストで完了。環境起因の一時的障害は再試行で解消する傾向。
