...
- **設計改善（2026-05-30 design-review）**: ~~① ColumnType 表示整形を `formatCellValue` に集約（PR #63）~~・~~② ViewQuery アイコンを `viewTypeConfig` に集約（PR #64）~~ 実装済。残: ③ 編集ウィジェットの JSX レジストリ化、project.store 責務分離、db-server PATCH 削除非対応。→ `outputs/queries/2026-05-30-design-review.md`
