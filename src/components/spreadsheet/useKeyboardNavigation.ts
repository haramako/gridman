...
export function useKeyboardNavigation({
  ...
  const handleContainerKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      switch (e.key) {
        ...
        case 'End':
          e.preventDefault()
          if (visibleColumns.length > 0) {
            setCursor({ rowId: cur.rowId, colKey: visibleColumns[visibleColumns.length - 1].key, tableName: cur.tableName })
          }
          break
    ...
    [filteredRows, visibleColumns, tableName, navigate, setCursor, extendCursor, setEditing, startEditWithInput, updateCell, handleCopy, handleCut, findDataEdgeRow, findDataEdgeCol, containerRef, containerHeight, readOnly, onSelectRows]
  )
